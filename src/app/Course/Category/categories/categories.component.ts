...
export class CategoriesComponent {
  ...
  getBackgroundColor(index: number): string {
    ...
    return colors[index % colors.length]; // Loops through the color array
  }
    ...
    if (confirm('Are you sure you want to delete this category?')) {
      this.categories.deleteCategory(categoryId).subscribe({
        next: (response) => {
          ...
          this.allCategories = this.allCategories.filter(category => category.id !== categoryId); // Remove from the list
        },
        error: (error) => {
